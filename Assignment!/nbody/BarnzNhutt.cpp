...
// #include <omp.h>
#include "Bhtree.cpp"
#include "calculate_duration.cpp"
#include "calculate_memory.cpp"
...
void runSimulation(struct body *b, char *image, double *hdImage) {
    ...
    for (int step = 1; step < STEP_COUNT; step++) {
        t.startTimer();
        m.caculateMemory();
        std::cout << "Beginning timestep: " << step;
        interactBodies(b);
...
void singleInteraction(struct body *a, struct body *b) {
    ...
    vec3 posDiffInteraction = vec3((a->position.x - b->position.x) * TO_METERS, (a->position.y - b->position.y) * TO_METERS, (a->position.z - b->position.z) * TO_METERS);
    int distance = magnitude(posDiffInteraction);
    int force = TIME_STEP * (G * a->mass * b->mass) / ((pow(distance, 2) + pow(SOFTENING, 2)) * distance);
    //change a accel
    vec3 bodyChangeA = vec3(force * posDiffInteraction.x / a->mass, force * posDiffInteraction.y / a->mass, force * posDiffInteraction.z / a->mass);
    a->accel.x -= bodyChangeA.x;
    a->accel.y -= bodyChangeA.y;
    a->accel.z -= bodyChangeA.z;
    //change b accel
    vec3 bodyChangeB = vec3(force * posDiffInteraction.x / b->mass, force * posDiffInteraction.y / b->mass, force * posDiffInteraction.z / b->mass);
    b->accel.x += bodyChangeB.x;
    b->accel.y += bodyChangeB.y;
    b->accel.z += bodyChangeB.z;
}
